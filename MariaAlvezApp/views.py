...
def relatorio_vacinacao(request):
    ...
    # --- NOVO CÓDIGO AQUI: Calcular days_diff para cada registro ---
    hoje = timezone.now().date() # Garante que 'hoje' esteja disponível
    for vacina in vacinacoes:
        if vacina.data_revacinacao:
            delta = vacina.data_revacinacao - hoje
            vacina.days_diff = delta.days
        else:
            vacina.days_diff = None # Ou algum valor que indique não definido
...
def relatorio_vermifugos(request):
    ...
    for vermifugo in vermifugos:
        if vermifugo.data_readministracao:
            ...
            vermifugo.days_diff = None # Ou algum valor que indique não definido
